...
%   locations for CA atom). If cType is set to 0, cFile is assumed to be a
...
%    ordi = [mystrfind('zoff', {p0.name})' setdiff(1:length(x0), [mystrfind('R1', {p0.name})' mystrfind('w1', {p0.name})'])];
    ordi = mymat2cell([mystrmatch('zoff', {p0.name}) mystrfind('dph0', {p0.name}) mystrfind('ph1', {p0.name})]);
...
%        p0 = addparam(p0, dz, sprintf('register Z-offset for chain %d (A)', ci), dz, dz, 'pri', 1);
        dz = absoluteToRegisterZoff(zoff(ci), R0, w0, a, w1, ph1(1), ph1(ci), cr(ci));
        p0 = addparam(p0, dz, sprintf('Z_off for chain %d (A)', ci), dz, dz, 'pri', 1);
...
switch upper(sym.type)
    ...
    case {'GENERAL'}
        n = size(M, 1); nc = n/sym.olig;
        XYZ = generateCrickBBrad(sym.olig, nc, r0, r1, w0, w1, a, ph1, cr, dph0, zoff);
end
...
function XYZ = generateCrickBBrad(chains, chL, r0, r1, w0, w1, a, ph1, cr, dph0, zoff, varargin)
