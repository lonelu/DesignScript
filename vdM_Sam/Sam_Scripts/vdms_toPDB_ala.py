...
for x in unique_clusters:
    ...
    for y in unique_vdMs:
        ...
        for z in unique_AA:
            ...
            score = df6['C'].iloc[0]
            np.savetxt("./output_ALA/%s_%s_%s_C=%.3f_%s_%s.pdb" % (file_AA, vdM_aa, clust_size, score, fname, fname2), df7, fmt=('%s','%6s','%3s','%4s','%1s','%3s','%11.3f','%7.3f','%7.3f','%5.2f','%5.2f','%6s','%4s')) # keep the "fmt" string to maintain PDB format correctly.

            filenames = ["./ala_SC_query.pdb", "./output_ALA/%s_%s_%s_C=%.3f_%s_%s.pdb" % (file_AA, vdM_aa, clust_size, score, fname, fname2)]
            
            with open("./output_ALA/%s_%s_%s_C=%.3f_%s_%s_helix.pdb" % (file_AA, vdM_aa, clust_size, score, fname, fname2), 'w') as outfile:
